...
struct RenderObject
{
    ...
    VkAccelerationStructureNVX bottomAS = VK_NULL_HANDLE;
...
public:
    ...
    void CreateAccelerationStructure(VkAccelerationStructureTypeNVX type, uint32_t geometryCount,
...
void TutorialApplication::CreateAccelerationStructure(VkAccelerationStructureTypeNVX type, uint32_t geometryCount,
    VkGeometryNVX* geometries, uint32_t instanceCount, VkAccelerationStructureNVX& AS, VkDeviceMemory& memory)
{
...
{
    VkGeometryNVX& geometry = object.geometry;
    ...
    CreateAccelerationStructure(VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NVX, 1, &geometry, 0,
...
void TutorialApplication::CreateAccelerationStructures()
{
...

    {
        ...
        {
            code = vkGetAccelerationStructureHandleNVX(_device, _renderObjects[i].bottomAS, sizeof(uint64_t), &accelerationStructureHandle);
        ...
        code = instanceBuffer.Create(instanceBufferSize, VK_BUFFER_USAGE_RAYTRACING_BIT_NVX, memoryFlags);
...

    {
        ...
        for (size_t i = 0; i < _renderObjects.size(); i++)
        {
            vkCmdBuildAccelerationStructureNVX(commandBuffer, VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NVX, 0, VK_NULL_HANDLE, 0,
                1, &_renderObjects[i].geometry, 0, VK_FALSE, _renderObjects[i].bottomAS, VK_NULL_HANDLE, scratchBuffer.Buffer, 0);
        ...
        vkCmdBuildAccelerationStructureNVX(commandBuffer, VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_NVX, _objectNum, instanceBuffer.Buffer, 0,
...
void TutorialApplication::CreatePipeline()
{
    ...
    std::vector<VkPipelineShaderStageCreateInfo> shaderStages
    {
        ...
        chitShaders[0].GetShaderStage(VK_SHADER_STAGE_CLOSEST_HIT_BIT_NVX),
        chitShaders[1].GetShaderStage(VK_SHADER_STAGE_CLOSEST_HIT_BIT_NVX),
    };
    ...
    const uint32_t groupNumbers[] = { 0, 1, 2, 3 };
...
void TutorialApplication::CreateShaderBindingTable()
{
    ...
    const VkDeviceSize shaderBindingTableSize = _raytracingProperties.shaderHeaderSize * groupNum;
...
void TutorialApplication::RecordCommandBufferForFrame(VkCommandBuffer commandBuffer, uint32_t frameIndex)
{
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_RAYTRACING_NVX, _rtPipelineLayout, 0,
...
void TutorialApplication::Cleanup()
{
    ...
    {
        if (object.bottomAS)
            vkDestroyAccelerationStructureNVX(_device, object.bottomAS, nullptr);
    ...
    if (_rtDescriptorSetLayouts[0])
    {
        vkDestroyDescriptorSetLayout(_device, _rtDescriptorSetLayouts[0], nullptr);
    }
    if (_rtDescriptorSetLayouts[1])
    {
        vkDestroyDescriptorSetLayout(_device, _rtDescriptorSetLayouts[1], nullptr);
    }
