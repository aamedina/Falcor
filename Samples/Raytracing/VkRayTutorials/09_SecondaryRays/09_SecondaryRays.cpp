...
{
    _appName = L"VkRay Tutorial 09: Secondary rays";
...
void TutorialApplication::CreateAccelerationStructures()
{
...

    {
        ...
        std::vector<Vertex> vertices
        {
            ...
            // Plane vertices
            { -1.0f + planeBiasX, planeBiasY, +1.0f },
            { +1.0f + planeBiasX, planeBiasY, +1.0f },
            { +1.0f + planeBiasX, planeBiasY, -1.0f },
            { -1.0f + planeBiasX, planeBiasY, -1.0f },
        };
...
void TutorialApplication::CreatePipeline()
{
...

    {
        ...
        ShaderResource chitShaderFirstRay;
        ShaderResource chitShaderSecondaryRay;
        ShaderResource ahitShaderSecondaryRay;
        ShaderResource missShadersFirstRay;
        ShaderResource missShadersSecondaryRay;

        LoadShader(rgenShader, L"rt_09_first.rgen.spv");
        LoadShader(chitShaderFirstRay, L"rt_09_first.rchit.spv");
        LoadShader(missShadersFirstRay, L"rt_09_first.rmiss.spv");
        LoadShader(chitShaderSecondaryRay, L"rt_09_secondary.rchit.spv");
        LoadShader(ahitShaderSecondaryRay, L"rt_09_secondary.rahit.spv");
        LoadShader(missShadersSecondaryRay, L"rt_09_secondary.rmiss.spv");

        std::vector<VkPipelineShaderStageCreateInfo> shaderStages(
            {
                rgenShader.GetShaderStage(VK_SHADER_STAGE_RAYGEN_BIT_NVX),
                chitShaderFirstRay.GetShaderStage(VK_SHADER_STAGE_CLOSEST_HIT_BIT_NVX),
                chitShaderSecondaryRay.GetShaderStage(VK_SHADER_STAGE_CLOSEST_HIT_BIT_NVX),
                ahitShaderSecondaryRay.GetShaderStage(VK_SHADER_STAGE_ANY_HIT_BIT_NVX),
                missShadersFirstRay.GetShaderStage(VK_SHADER_STAGE_MISS_BIT_NVX),
                missShadersSecondaryRay.GetShaderStage(VK_SHADER_STAGE_MISS_BIT_NVX),
            });
...
void TutorialApplication::RecordCommandBufferForFrame(VkCommandBuffer commandBuffer, uint32_t frameIndex)
{
    ...
    vkCmdTraceRaysNVX(commandBuffer,
        ...
        _shaderBindingTable.Buffer, 3 * _raytracingProperties.shaderHeaderSize, _raytracingProperties.shaderHeaderSize,
